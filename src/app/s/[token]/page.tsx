...
export async function generateMetadata({
  ...
  const resolvedParams = await (params ?? Promise.resolve({ token: "" }));
  const session = findUnlistedSessionByToken(resolvedParams.token, new Date());
  ...
  return {
    ...
    description: `${session.courseTitle} session shared privately with a link.`,
  };
}

export default async function UnlistedSessionPage({ params }: PageProps) {
  const resolvedParams = await (params ?? Promise.resolve({ token: "" }));
  const session = findUnlistedSessionByToken(resolvedParams.token, new Date());
